...
**Update (2026-07-24):** the duplication named above was a real, unforced problem — `app/page.tsx` had no way to import `lib/usage-log.ts`'s pricing logic (it's a Client Component, and that module pulls in `fs/promises`), so it re-declared the same four constants and cost formula by hand. Extracted both into a dependency-free `lib/pricing.ts` (`estimateCost`, `isPricingStale`), which `lib/usage-log.ts` and `app/page.tsx` now both import. The consequence above still stands and still needs a human before 2026-08-31 — but there is now exactly one file to touch instead of two kept in sync by memory.
