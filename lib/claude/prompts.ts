...
3. Return ONLY a valid JSON object matching the schema given in the task instructions. No markdown, no explanation, no code fences. This holds even when the job description is a poor match for the profile (e.g. a sales role against an engineering background): do not preface or follow the JSON with caveats, disclaimers, or a recommendation against applying. Do your most honest, non-fabricated tailoring within the JSON's own fields (the summary and bullet selection are where a weak fit legitimately shows up) and say nothing outside it — surfacing fit concerns to the candidate is the calling application's job, not yours.
...
2. The entire resume MUST fit on ONE PAGE. Select exactly 2 experience entries and exactly 3 projects. Include exactly 2–3 bullets per experience entry and exactly 2 bullets per project. Show only the 2 most recent education entries. Keep the summary to 2 sentences and under 50 words total. Include up to 4 certifications most relevant to the role. Omit the certifications field entirely if none are relevant.
...
4. MARKET-AWARE LENGTH: rule 2's one-page, 2-experience/3-project split assumes a US/UK tech-company audience, where hiring managers weight shipped technical projects over unrelated work history for entry-level candidates. If the job posting signals a Malaysian, Singaporean, or other Southeast Asian/Commonwealth banking or graduate-programme employer (the same signals as the cover-letter MARKET-AWARE TONE rule: "Berhad", "GLC", "graduate programme", a Malaysian/Singaporean location, banking/finance sector language), the emphasis flips: these employers evaluate leadership, stakeholder management, and organizational experience over technical project depth, and a longer resume is the norm there even for entry-level candidates, not a red flag. Allow up to 5 experience entries and up to 2 projects, spanning up to two pages, still selected and ordered by the same RELEVANCE CRITERIA below — experience entries should lead and dominate the page, with projects as a minor supplement. Do not stretch content to fill the extra space, only include additional entries that are genuinely relevant, and default back to rule 2's strict one-page/2-experience/3-project split whenever the posting doesn't clearly signal this market.
...
1. Malaysia: no visa constraint. Any legitimate SWE-adjacent employer or structured graduate programme is in scope.
2. Singapore and United Kingdom: ONLY include employers you can confirm via search sponsor work visas for early-career hires in that country (Singapore Employment Pass sponsorship, UK Skilled Worker sponsorship). For the UK, the candidate holds a US degree and is based abroad, so they do NOT qualify for the UK Graduate Visa (PSW) — the employer must directly sponsor a Skilled Worker visa. Every listing's "note" field must reflect that sponsorship signal (e.g. what confirmed it).
...
4a. LINKS MUST BE COMPANY-DIRECT ONLY: every url must point to that employer's own domain (e.g. company.com/careers) or a single-company hosted apply page on an ATS (e.g. jobs.ashbyhq.com/companyname, boards.greenhouse.io/companyname, company.wd105.myworkdayjobs.com) — never a multi-employer search/aggregator platform (Jobstreet, Glassdoor, Indeed, LinkedIn Jobs, Prosple, BuiltIn, Hiredly, or any other site where the candidate would search across many employers, even when the URL is filtered to one company or role). The candidate browses those platforms himself and does not want them duplicated here. If you can't find a company-direct or single-company-ATS link for an employer, drop that employer rather than substituting an aggregator link.
...
6. Write one 1 to 2 sentence "blurb" per country in the same voice as: "No visa constraint. Excludes every company already on the candidate's active application list." (Malaysia) / "Sponsorship-confirmed employers only, excluding every company already on the candidate's active application list." (Singapore and United Kingdom)
